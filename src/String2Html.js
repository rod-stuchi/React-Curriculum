...
export const string2Html = str => {
  return str
    .split(/(<br>)/)
    .map(x => (/<br>/.test(x) ? <br key={v4()} /> : urlTemplate(x)));
};

const urlTemplate = str => {
  return str.split(/(<[^|]+\|[^>]+>)/g).map(
    x =>
      /^<[^>]+>/.test(x)
        ? (() => {
            let m = x.match(/^<([^|]+)\|([^>]+)>/);
            return (
              <Link key={v4()} href={m[2]}>
                {m[1]}
              </Link>
            );
          })()
...
const boldTemplate = str => {
  return str.split(/(\*{2}[^\*]+\*{2})/).map(
    x =>
      /\*{2}[^\*]+\*{2}/.test(x)
        ? (() => {
            let m = x.match(/\*{2}([^\*]+)\*{2}/);
            return <span style={{ fontWeight: "800" }}>{m[1]}</span>;
          })()
        : italicTemplate(x)
  );
};
  ...
  return str.split(/(\*[^\*]+\*)/).map(
    x =>
      /\*[^\*]+\*/.test(x)
        ? (() => {
            let m = x.match(/\*([^\*]+)\*/);
            return <span style={{ fontStyle: "italic" }}>{m[1]}</span>;
          })()
        : emphasisTemplate(x)
  );
};
